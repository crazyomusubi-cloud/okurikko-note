...
async function resolveSelections(selections){
  ...
  for(const s of selections){
    if(s.id){ ids.push(s.id); }
      ...
      const ref = await addDoc(collection(db,'people'), { name: s.newName, note: '', createdAt: serverTimestamp() });
      ids.push(ref.id);
    }
  }
  return ids;
...
function attachListeners(){
  unsubs.push(onSnapshot(collection(db,'people'), (snap)=>{
    allPeople = snap.docs.map(d=>({id:d.id, ...d.data()}));
    refreshCurrentView();
  }));
  unsubs.push(onSnapshot(query(collection(db,'records'), orderBy('date','desc')), (snap)=>{
    allRecords = snap.docs.map(d=>({id:d.id, ...d.data()}));
    refreshCurrentView();
  }));
  unsubs.push(onSnapshot(collection(db,'stock'), (snap)=>{
